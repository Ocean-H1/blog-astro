...
function replaceCustomContainersToMarkdownQuote(
	...
	processedContent = processedContent.replace(
		...
		(_match: string, _containerType: string, content: string) => {
			const trimmedContent: string = content.trim().replace(/\n/g, "\n> ");
	...
	processedContent = processedContent.replace(
		/::github\s*\{\s*repo\s*=\s*(["'])([^'"]+)\1\s*\}/gi,
		(match: string, _quote: string, repo: string) => {
			const trimmedRepo = repo.trim();
			if (!trimmedRepo) return match;
			return `[GitHub Repository(${trimmedRepo})](https://github.com/${trimmedRepo})`;
		},
	);
