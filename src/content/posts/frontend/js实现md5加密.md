...
最近在写一个项目的登录功能时，出现了一个需求，要将用户的密码进行`md5`加密成 16 位后再进行传输

查了一些资料，找到了`js-md5`这个库，可以通过其中提供的`api`直接对数据进行加密，但是只能加密成 32 位，研究了半天也没找到解决办法
...
> 创建 utils/utils.js，里面存放封装的一些工具函数
